...
export class LocationsComponent extends BaseClientComponent implements OnInit {
  ...
  initializeLocationModel(data: any = null, parentGroupId: any = null) {
    if(data != null) {
      this.locationModel = {
        _id: data._id,
  ...
  openLocationPopup(data: any = null, parentGroupId: any = null) {
    this.initializeLocationModel(data, parentGroupId);
        ...
        if (response.success == true) {
          ...
          switch(request.action) {
            ...
            case 'UPDATE_LOCATION':
              this.companyLocations = this.modifyLocation(this.companyLocations, request);
              this.messageObject = this.messageService.successMessageObject(successMessages.LOCATION_UPDATED_SUCCESS);
            break;
  ...
  /* For Modify Location */
  modifyLocation(locations, data) {
    locations.forEach(location => {
      if(this.isFound) return;
      if(String(location._id) ==  String(data.ParentGroupId)) {
        let index = location.Locations.findIndex(x => String(x._id) == String(data._id));
        if(index !== -1) {
          location.Locations[index].LocationName = data.LocationName;
          location.Locations[index].ContactName = data.ContactName;
          location.Locations[index].Email = data.Email;
          location.Locations[index].Phone = data.Phone;
          location.Locations[index].Address = data.Address;
        } 
        this.isFound = true;
      } else if(location.Children && location.Children.length > 0){
        location.Children = this.modifyLocation(location.Children, data);
      }
    });
